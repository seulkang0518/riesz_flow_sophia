...
def _sinkhorn_effective_neighbor_diagnostics(
    ...
    # Match drift_loss_ot's scale normalization as closely as possible.
    ...
    if use_quadratic_cost:
        ...
        scale = (weighted_dist_sq.mean() / targets_w.mean().clamp_min(eps)).sqrt()
    else:
        weighted_dist = dist * targets_w[:, None, :]
        scale = weighted_dist.mean() / targets_w.mean().clamp_min(eps)
                ...
                if _use_ot and _sinkhorn_eff_enabled and _sinkhorn_eff_count < _sinkhorn_eff_max_features:
                    if _use_new_cfg:
                        _diag_cfg_w = repeat(
                            chunk_cfg - 1.0,
                            "b -> (b f)",
                            f=Bf // max(1, chunk_cfg.shape[0]),
                        )
                        _diag_fixed_neg = torch.cat([feature_neg_detached, feature_uncond], dim=1)
