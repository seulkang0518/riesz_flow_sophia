...
def laplace_unit_field_loss(
    ...
    with torch.no_grad():
        ...
        V = V_pos + V_gen_rep + V_neg_rep

        if include_inverse_tau:
            V = V / float(tau)
        V = V * float(field_scale)

        # Field is expressed in unit directions, which are unchanged by the
        # isotropic feature scaling used for the fixed-point surrogate below.
        goal_scaled = old_gen / scale_inputs + V
        ...
        info: Dict[str, torch.Tensor] = {
            ...
            "laplace_total_field_rms": V.square().mean().sqrt(),
