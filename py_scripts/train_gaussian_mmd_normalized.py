...
from gaussian_mmd_loss_normalized import gaussian_mmd_loss
...
def train_step(
    ...
    for accum_idx in range(actual_accum):
        ...
        if _use_riesz and _use_gaussian_mmd:
            raise ValueError("use_riesz and use_gaussian_mmd cannot both be true")
        _use_particle_field = _use_riesz or _use_gaussian_mmd
            ...
            for k in chunk_sg.keys():
                ...
                if _use_particle_field:
                    ...
                    if _use_gaussian_mmd:
                        gmmd_kwargs = dict(gaussian_mmd_kwargs or {})
                        gmmd_kwargs["current_step"] = int(state.step)
                        gmmd_kwargs["calibration_key"] = str(k)
                        loss_feat, info = gaussian_mmd_loss(
                            gen=feature_gen,
