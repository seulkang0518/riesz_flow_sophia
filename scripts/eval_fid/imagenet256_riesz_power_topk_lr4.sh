...
export CONFIG=${CONFIG:-configs/gen/imagenet256_riesz_power_topk_lr4.yaml}
export RUN_DIR=${RUN_DIR:-/SAN/intelsys/imagenet_mmd_flow/imagenet256_riesz_power_topk_lr4_1node_4gpu_official30k}
export CKPT_DIR=${CKPT_DIR:-$RUN_DIR/checkpoints}
export WORK_ROOT=${WORK_ROOT:-$RUN_DIR/fid_inference_ours_cfg1p5_work}
export OUT_ROOT=${OUT_ROOT:-$RUN_DIR/fid_inference_ours_cfg1p5_json}
export FID_REF=${FID_REF:-$WFLOW_HF_ROOT/stats/jit_in256_stats.npz}
# ----------------------------------------

export CFG_SCALE=${CFG_SCALE:-1.5}
...
for CKPT in "$CKPT_DIR"/state_*.pt; do
  if [[ ! -f "$CKPT" ]]; then
    continue
  fi
