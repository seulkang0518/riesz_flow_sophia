...
# One GPU requested above.
export NGPU=${NGPU:-${NSLOTS:-1}}
export MASTER_PORT=${MASTER_PORT:-28817}

# Same YAML and same training run/checkpoint folder.
export CONFIG=${CONFIG:-configs/gen/imagenet64_riesz_minyuan_power_top20.yaml}
export RUN_DIR=${RUN_DIR:-/SAN/intelsys/imagenet_mmd_flow/imagenet64_riesz_minyuan_power_top20}
export CKPT_DIR=${CKPT_DIR:-$RUN_DIR/checkpoints}

# Separate output name for CFG 2.0 evaluation.
export CFG_SCALE=${CFG_SCALE:-2.0}
export EVAL_TAG=${EVAL_TAG:-cfg2p0}
export WORK_ROOT=${WORK_ROOT:-$RUN_DIR/fid_inference_ours_${EVAL_TAG}_work}
export OUT_ROOT=${OUT_ROOT:-$RUN_DIR/fid_inference_ours_${EVAL_TAG}_json}

# ImageNet64 FID reference.
export FID_REF=${FID_REF:-$WFLOW_HF_ROOT/stats/VIRTUAL_imagenet64_labeled.npz}
...
for STEPNUM in 00120000; do
  ...
  JSON_OUT="$OUTDIR/results_${EVAL_TAG}.json"
...
if [[ "$found_ckpt" == "0" ]]; then
  echo "Error: none of the requested checkpoints were found in $CKPT_DIR" >&2
  exit 1
fi
