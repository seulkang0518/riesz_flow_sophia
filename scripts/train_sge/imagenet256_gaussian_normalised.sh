...
if [[ ! -f train_gaussian_mmd.py ]]; then
  echo "Error: train_gaussian_mmd.py was not found in repository directory: $REPO_DIR" >&2
...
export CONFIG=${CONFIG:-configs/gen/gaussian_mmd_normalized_config_patch.yaml}
export RUN_NAME=${RUN_NAME:-imagenet256_gaussian_mmd_normalised_1node_4gpu_official30k}
...
torchrun \
  ...
  train_gaussian_mmd_normalized.py \
  --config "$CONFIG" \
  --workdir "$WORKDIR"
