...
def _weighted_off_diagonal_pair_mean(
    ...
    return weighted.sum(dim=(-1, -2)) / float(
        n_left * (n_left - 1)
    )
...
def riesz_loss(
    ...
    *,
    ...
    Here:
        c_self = self_repulsion_weight * base_self_repulsion_coefficient.

    Self-support modes are selected by the caller:

    * ``self_support=None``:
      Use the current generated batch on both sides.

      With ``use_unbiased_same_batch=True``, the self-term excludes diagonal
      pairs and is normalised by N(N-1).

      Gradients pass through both generated sides, so the base self-repulsion
      coefficient is 1.

    * ``self_support=tensor``:
      Use a detached external support, such as a fresh independent generated
      batch or a generated-feature memory bank.

      Only the live ``gen`` side receives gradients, so the base self-repulsion
      coefficient is 2 to preserve the two-sided force scale when the external
      support approximates the current generator distribution.

    Inputs have shape [B, particles, features].
    The returned loss has shape [B].
    """
    if epsilon <= 0:
        raise ValueError("epsilon must be positive")

    self_repulsion_weight = float(self_repulsion_weight)
    if self_repulsion_weight < 0:
        raise ValueError("self_repulsion_weight must be non-negative")

    if gen.ndim != 3 or fixed_pos.ndim != 3:
        raise ValueError(
            "gen and fixed_pos must have shape [B, particles, D]"
        )

    if gen.shape[0] != fixed_pos.shape[0]:
        raise ValueError(
            "gen and fixed_pos must have the same batch dimension"
        )
    ...
    if self_support is None:
        ...
        if self_support.ndim != 3:
            raise ValueError(
                "self_support must have shape [B, particles, D]"
            )

        if self_support.shape[0] != gen.shape[0]:
            raise ValueError(
                "self_support and gen must have the same batch size"
            )

        if self_support.shape[-1] != gen.shape[-1]:
            raise ValueError(
                "self_support and gen must have the same feature dimension"
            )

        if self_support.shape[1] == 0:
            raise ValueError(
                "self_support must contain at least one particle"
            )

        if weight_self_support is None:
            weight_self_support = torch.ones_like(
                self_support[:, :, 0]
            )
    ...
    if external_self_support:
        ...
        self_support = self_support.float()
    ...
    if external_self_support:
        ...
        base_self_repulsion_coefficient = 2.0
        ...
        base_self_repulsion_coefficient = 1.0
    ...
    effective_self_repulsion_coefficient = (
        self_repulsion_weight
        * base_self_repulsion_coefficient
    )

    distance_pos_pos = torch.cdist(
        pos_scaled,
        pos_scaled,
    )
    ...
    if neg_scaled.shape[1] > 0:
        ...
        fixed_negative_repulsion = torch.zeros_like(
            attraction
        )

    loss = (
        2.0 * attraction
        - effective_self_repulsion_coefficient * self_repulsion
        - target_repulsion
    ...
    info = {
        ...
        "riesz_self_repulsion": (
            self_repulsion.detach().mean()
        ),

        "riesz_self_repulsion_base_coefficient": torch.tensor(
            base_self_repulsion_coefficient,
            device=gen.device,
        ),

        "riesz_self_repulsion_weight": torch.tensor(
            self_repulsion_weight,
            device=gen.device,
        ),
        ...
        "riesz_target_repulsion": (
            target_repulsion.detach().mean()
        ),
        ...
        "riesz_external_self_support": torch.tensor(
            float(external_self_support),
            device=gen.device,
        ),

        "riesz_self_support_size": torch.tensor(
            float(self_support.shape[1]),
            device=gen.device,
        ),
    }
