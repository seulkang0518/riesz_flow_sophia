...
from drift_loss import drift_loss, cdist as drift_cdist
from drift_loss_ot import drift_loss_ot, _sinkhorn_plan_log
...
@torch.no_grad()
def _sinkhorn_effective_neighbor_diagnostics(
    gen: torch.Tensor,
    ...
    prefix: str = "sinkhorn_eff_pos",
):
    """
    Diagnostic for OT/Sinkhorn locality.

    For each generated sample i, compute the Sinkhorn transport plan from
    generated samples to positive samples, row-normalize it into p_ij, then log:

        effective_neighbors_i = 1 / sum_j p_ij^2

    and top-k mass concentration. If effective_neighbors is around 4 and/or
    top4_mass is high, Sinkhorn is effectively behaving like a soft top-4 method.

    This is diagnostic-only and is detached from the training graph.
    """
    eps = 1e-12
    info = {}
    ...
    _, _, feature_dim = gen.shape

    # Match drift_loss_ot's feature-scale normalization exactly.
    targets = torch.cat([gen, fixed_neg, fixed_pos], dim=1)
    ...
    reg_scale = float(feature_dim) if use_quadratic_cost else float(feature_dim) ** 0.5

    for R in tuple(float(r) for r in R_list):
        reg = torch.tensor(R * reg_scale, device=gen.device, dtype=gen_scaled.dtype)
        ...
        pi = _sinkhorn_plan_log(
            ...
            stop_thr=float(sinkhorn_stop_thr),
            target_weights=None,
            use_quadratic_cost=bool(use_quadratic_cost),
        )

        p = pi / pi.sum(dim=-1, keepdim=True).clamp_min(eps)

        eff = 1.0 / p.pow(2).sum(dim=-1).clamp_min(eps)
...
@torch.no_grad()
def _drift_effective_neighbor_diagnostics(
    gen: torch.Tensor,
    fixed_pos: torch.Tensor,
    fixed_neg: torch.Tensor | None = None,
    weight_gen: torch.Tensor | None = None,
    ...
    # Match drift_loss exactly: same targets, weights, distance routine, scale,
    # normalized distances, and generated-self diagonal mask.
    targets = torch.cat([gen, fixed_neg, fixed_pos], dim=1)
    targets_w = torch.cat([weight_gen, weight_neg, weight_pos], dim=1)

    dist = drift_cdist(gen, targets)
    weighted_dist = dist * targets_w[:, None, :]
    scale = weighted_dist.mean() / (targets_w.mean() + 1e-8)
...
def train_step(
    ...
    drift_eff_kwargs: dict | None = None,
):
    ...
    for accum_idx in range(actual_accum):
        ...
        _use_drift = (not _use_riesz) and (not _use_ot)
        _ot_kw = ot_kwargs or {}
        _drift_eff_kw = drift_eff_kwargs or {}
        ...
        with sync_ctx:
            ...
            _sinkhorn_eff_every = int(_ot_kw.get("sinkhorn_eff_diag_every", 0) or 0)
            _sinkhorn_eff_max_features = int(_ot_kw.get("sinkhorn_eff_diag_max_features", 999999) or 999999)
            _sinkhorn_eff_enabled = (
                _use_ot
                and _sinkhorn_eff_every > 0
                and (int(state.step) % _sinkhorn_eff_every == 0)
            )
            _sinkhorn_eff_count = 0

            _drift_eff_every = int(_drift_eff_kw.get("diag_every", 0) or 0)
            _drift_eff_max_features = int(_drift_eff_kw.get("diag_max_features", 999999) or 999999)
            _drift_eff_enabled = (
                _use_drift
                and _drift_eff_every > 0
                and (int(state.step) % _drift_eff_every == 0)
            )
            _drift_eff_count = 0
            ...
            for k in chunk_sg.keys():
                ...
                if _use_drift and _drift_eff_enabled and _drift_eff_count < _drift_eff_max_features:
                    eff_info = _drift_effective_neighbor_diagnostics(
                        gen=feature_gen,
                        fixed_pos=feature_pos,
                        fixed_neg=feature_uncond,
                        weight_gen=torch.ones_like(feature_gen[:, :, 0]),
                        weight_pos=torch.ones_like(feature_pos[:, :, 0]),
                        weight_neg=weight_neg,
                        R_list=loss_kwargs.get("R_list", (0.02, 0.05, 0.2)),
                        prefix="drift_eff_pos",
                    )
                    info.update(eff_info)
                    _drift_eff_count += 1

                if _use_ot and _sinkhorn_eff_enabled and _sinkhorn_eff_count < _sinkhorn_eff_max_features:
                    if _use_new_cfg:
                        # Match drift_loss_ot's scale calculation: fixed_uncond is
                        # passed separately for the CFG barycentric term and is
                        # not included in the feature-scale normalization.
                        _diag_fixed_neg = feature_neg_detached
                        _diag_weight_neg = torch.ones_like(feature_neg_detached[:, :, 0])
                    else:
                        _diag_fixed_neg = ot_neg
                        _diag_weight_neg = ot_neg_w

                    eff_info = _sinkhorn_effective_neighbor_diagnostics(
                        gen=feature_gen,
                        fixed_pos=feature_pos,
                        fixed_neg=_diag_fixed_neg,
                        weight_gen=torch.ones_like(feature_gen[:, :, 0]),
                        weight_pos=torch.ones_like(feature_pos[:, :, 0]),
                        weight_neg=_diag_weight_neg,
                        R_list=ot_loss_kwargs.get("R_list", loss_kwargs.get("R_list", (0.02, 0.05, 0.2))),
                        sinkhorn_num_iter=ot_loss_kwargs.get("sinkhorn_num_iter", 20),
                        sinkhorn_stop_thr=ot_loss_kwargs.get("sinkhorn_stop_thr", 1e-4),
                        use_quadratic_cost=ot_loss_kwargs.get("use_quadratic_cost", False),
                        prefix="sinkhorn_eff_pos",
                    )
                    info.update(eff_info)
                    _sinkhorn_eff_count += 1
        ...
        if step == initial_step:
            profile_metrics = profile_func(
                lambda s, l, p, n, fp: train_step(
                    ...
                    drift_eff_kwargs=drift_eff_kwargs,
                    **forward_dict,
                ),
